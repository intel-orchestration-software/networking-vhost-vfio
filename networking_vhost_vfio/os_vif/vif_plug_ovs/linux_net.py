...
from networking_vhost_vfio.os_vif.internal.command import ip as ip_lib
...
from networking_vhost_vfio.os_vif.vif_plug_ovs import privsep
...
@privsep.vif_plug.entrypoint
def create_vlan(parent, vlan_id):
    try:
        processutils.execute('ip', 'link', 'show',  parent + ".%s" % (vlan_id),
                             check_exit_code=[0])
        LOG.debug("device %s already exists and cannot be created",
                  parent + ".%s" % (vlan_id))
    except:
        processutils.execute('ip', 'link', 'add', 'link', parent, 'name',
                             parent + ".%s" % (vlan_id),
...
def delete_vlan(dev):
    try:
        processutils.execute('ip', 'link', 'show', dev,
                             check_exit_code=[1])
        processutils.execute('ip', 'link', 'del', dev)
    except:
        LOG.debug("device %s does not exist and cannot be deleted", dev)
