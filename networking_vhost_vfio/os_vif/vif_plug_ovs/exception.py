...
from networking_vhost_vfio.os_vif import exception as osv_exception
...
class MissingPortProfile(osv_exception.ExceptionBase):
    msg_fmt = ('A port profile is mandatory for the OpenVSwitch plugin')


class WrongPortProfile(osv_exception.ExceptionBase):
    msg_fmt = ('Port profile %(profile)s is not a subclass '
               'of VIFPortProfileOpenVSwitch')


class RepresentorNotFound(osv_exception.ExceptionBase):
    msg_fmt = ('Failed getting representor port for PF %(ifname)s with '
               '%(vf_num)s')
