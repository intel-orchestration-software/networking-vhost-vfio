...
from networking_vhost_vfio.os_vif.vif_plug_ovs import constants
from networking_vhost_vfio.os_vif.vif_plug_ovs import exception
from networking_vhost_vfio.os_vif.vif_plug_ovs import linux_net


class VirtioDirectPlugin(plugin.PluginBase):
    ...
    CONFIG_OPTS = (
        ...
        cfg.IntOpt('ovs_vsctl_timeout',
                   ...
                   'forever.',
                   deprecated_group="DEFAULT"),
        cfg.IntOpt('ovs_vlan_offset',
                   default=128,
                   help='The vlan offset for binding a vnic_type direct port'),
    )

    @staticmethod
    def gen_port_name(prefix, id):
        return ("%s%s" % (prefix, id))[:VirtioDirectPlugin.NIC_NAME_LEN]

    @staticmethod
    def get_veth_pair_names(vif):
        return (VirtioDirectPlugin.gen_port_name("qvb", vif.id),
                VirtioDirectPlugin.gen_port_name("qvo", vif.id))

    def describe(self):
        pp_ovs = objects.host_info.HostPortProfileInfo(
            profile_object_name=objects.vif.
            VIFPortProfileOpenVSwitch.__name__,
            min_version="1.0",
            max_version="1.0",
        )
        pp_ovs_representor = objects.host_info.HostPortProfileInfo(
            profile_object_name=objects.vif.
            VIFPortProfileOVSRepresentor.__name__,
            min_version="1.0",
            max_version="1.0",
        )
    ...
    def _plug_vhostuser(self, vif, instance_info):
        ...
        vif_name = VirtioDirectPlugin.gen_port_name(
    ...
    def _plug_vf_passthrough(self, vif, instance_info):
        ...
        representor = "eth0.%i" % (int(vf_num) + self.config.ovs_vlan_offset)
        linux_net.create_vlan("eth0", int(vf_num) +
                                      self.config.ovs_vlan_offset)
    ...
    def _unplug_vhostuser(self, vif, instance_info):
        linux_net.delete_ovs_vif_port(vif.network.bridge,
                                      VirtioDirectPlugin.gen_port_name(
    ...
    def _unplug_vf_passthrough(self, vif, instance_info):
        ...
        linux_net.delete_vlan(representor)
