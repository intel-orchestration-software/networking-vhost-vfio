...
def list_device_types(dev):
    ...
    dev_types = []
    dev_path = os.path.join(SYS_PATH, dev, "mdev_supported_types")
...
def device_type_details(dev, type_id):
    ...
    dev_path = os.path.join(
        SYS_PATH, dev, "mdev_supported_types", type_id)
...
def create_device_type(dev, type_id, dev_uuid):
    ...
    create_file = os.path.join(
        SYS_PATH, dev, "mdev_supported_types", type_id, "create")

    try:
        LOG.info("Creating a new mdev: %s", dev_uuid)
        with os.open(create_file, 'a') as create:
...
def list_devices(dev, type_id):
    ...
    devices = []
    dev_path = os.path.join(
        SYS_PATH, dev, "mdev_supported_types", type_id, "devices")
...
def available_devices(dev, type_id):
    ...
    try:
        ...
        with os.open(instances_file) as file:
            instances = file.read()
    except Exception as exc:
...
def device_type_api(dev, type_id):
    ...
    api_file = os.path.join(
        SYS_PATH, dev, "mdev_supported_types", type_id, "device_api")

    try:
        LOG.info("Getting API of mdev type, %s", type_id)
        with os.open(api_file) as file:
            api = file.read()
    except Exception as exc:
...
def device_type_name(dev, type_id):
    ...
    name_file = os.path.join(
        SYS_PATH, dev, "mdev_supported_types", type_id, "name")

    try:
        LOG.info("Getting name of mdev type, %s", type_id)
        with os.open(name_file) as file:
            name = file.read()
    except Exception as exc:
...
def device_type_description(dev, type_id):
    ...
    description_file = os.path.join(
        SYS_PATH, dev, "mdev_supported_types", type_id, "description")

    try:
        LOG.info("Getting description of mdev type, %s", type_id)
        with os.open(description_file) as file:
...
def device_attributes(dev, type_id, uuid):
    ...
    attribute_path = os.path.join(
        SYS_PATH, dev, "mdev_supported_types", type_id, "devices", uuid)
...
def device_type(dev, type_id, uuid):
    ...
    try:
        ...
        with os.open(type_file) as file:
            mdev_type = file.read()
    except Exception as exc:
...
def remove_device(dev, type_id, uuid):
    ...
    try:
        ...
        with os.open(remove_file, 'w+') as remove:
            remove.write('1')
    except Exception as exc:
